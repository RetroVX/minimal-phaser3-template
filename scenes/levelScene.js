...
export default class levelScene extends Phaser.Scene {
  ...
  create() {
    ...
    const playerY = this.camera.height;
    ...
    // score
    this.score = 0;
    this.scoreText = this.add.text(this.camera.width /2, 24, '0', { font: '36px Ariel', fill: '#FFFFFF' });
    this.scoreText.setOrigin(0.5, 0.5);

    // collide player and the ball with arcade physics
    // when the player and ball collide, run the spriteCollision function
    this.physics.add.collider(this.player.sprite, this.ball, this.spriteCollision, null, this);
